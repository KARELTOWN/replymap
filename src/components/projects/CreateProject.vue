<template>
  <ModalShell
    v-if="isOpen"
    :title="selectProject ? 'Modifier le projet' : 'Ajouter un projet'"
    :description="
      selectProject
        ? 'Nom, adresse suivie et données collectées.'
        : 'Un projet correspond à un site à suivre.'
    "
    @close="closeModal"
  >
    <form id="project-form" class="space-y-5" @submit.prevent="handleSubmit">
      <div>
        ...
        <input
          ...
          v-model="libelle"
          type="text"
          placeholder="Ex : Site vitrine du client"
          :class="fieldClass"
        />
      ...
      <div>
        ...
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          C'est le domaine autorisé à envoyer des données. Il ne pourra plus changer une fois la
          collecte commencée.
        </p>
      ...
      <div
        v-if="tracking_code && !selectProject"
      ...
      >
        <div class="flex items-start justify-between gap-3">
          ...
          <button
            type="button"
            @click="copyScript(tracking_code)"
        ...
        <pre
          class="mt-3 overflow-x-auto rounded-lg bg-gray-900 p-3 text-xs leading-relaxed text-gray-100"
          >{{ tracking_code }}</pre
        >
      </div>
        ...
        >
          ...
          >
            <input
              ...
              :checked="track[option.key] !== false"
...
// What each collection does, said in the user's terms rather than in the field
// names of the API — and, above all, what it really collects: JavaScript errors
// were announced here while they in fact depended on another box, and slowness
// promised "tasks" that were never reported. "Behaviours" is gone: what it
// covered only makes sense inside a session, so it follows the recording.
const trackingOptions = [
  {
    key: 'active_recording',
    title: 'Enregistrement des sessions',
    description: 'Rejouez le parcours du visiteur, écran par écran.',
  },
  {
    key: 'active_track_errors',
    title: 'Erreurs',
    description: 'Erreurs JavaScript, promesses rejetées et requêtes en échec.',
  },
  {
    key: 'active_performance_issues',
    title: 'Lenteurs',
    description: 'Requêtes réseaux de votre application qui dépassent trois secondes.',
  },
]
