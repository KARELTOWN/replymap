<template>
  ...
  <div class="rounded-xl border border-gray-200 dark:border-gray-800">
    ...
    <!-- Where the script actually runs. A project declares one domain, but
             the snippet is copied by hand: it ends up on a staging site, a
             second domain, sometimes a site it was never meant for.
             This block sits on a surface of its own and each site is a framed
             row: buried in the diagnostics list, one could no longer see it was
             an inventory, nor that it could be acted upon. -->
    <section
      v-if="hosts.length"
    ...
    >
      ...
      <ul class="space-y-2">
        ...
        >
          <div class="flex min-w-0 items-center gap-2">
            ...
            <span
              v-else-if="isDeclared(entry.host)"
          ...
          <div class="ml-6 flex shrink-0 items-center gap-3 sm:ml-0">
            ...
            <!-- Cuts the script off on this site: it no longer
                             starts at all, feedback widget included. -->
            <button
              type="button"
              role="switch"
              :aria-checked="!entry.blocked"
              :aria-label="`Collecte sur ${entry.host}`"
              :title="
                entry.blocked
                  ? `Réactiver la collecte sur ${entry.host}`
                  : `Désactiver la collecte sur ${entry.host}`
              "
              :disabled="hostUpdating === entry.host"
              @click="toggleHost(entry)"
            ...
            >
              <span
                ...
                :class="entry.blocked ? 'left-0.5' : 'left-4.5'"
              ></span>
            </button>
          </div>
        </li>
      ...
      <p class="mt-3 text-[11px] leading-relaxed text-gray-500 dark:text-gray-400">
        Désactiver un site y arrête le script entièrement — enregistrement, évènements et widget de
        feedback — sans toucher aux autres sites.
      </p>
    </section>
...
const reasonText = (reason: any): string => {
  switch (reason.code) {
    ...
    case 'COLLECTION_OFF':
      // The feedback widget runs whatever happens: saying the script
      // sends nothing at all would be false.
...
const toggleHost = (entry: any) => setHostBlocked(props.projectId, entry.host, !entry.blocked)
    ...
    const message =
      {
        ...
        ADDRESS_REFUSED:
          'Cette adresse ne peut pas être vérifiée depuis le serveur (adresse locale ou privée).',
        UNREACHABLE:
          'Le site n’a pas répondu. Vérifiez qu’il est en ligne et accessible publiquement.',
